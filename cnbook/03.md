...
注意 网络| |关于网络主题的信息量要多得多，这超出了本书的范围。如果您感兴趣，我鼓励您进一步研究这个主题。|
...
您可能会感到惊讶，但是在Nim开始一个项目非常快速和容易。只需打开您喜欢的文本编辑器，创建一个新文件并开始编码。但在执行此操作之前，您应该确定项目的目录布局。这完全是可选的，编译器不会介意您是否将所有代码保存在 中，但这样做是不好的做法。理想情况下，您应该只为此项目创建一个新目录，例如 （或 ）。在该目录中创建一个目录来存储所有源代码。将来，如有必要，可以在项目中创建其他目录，例如 、 等。大多数其他Nim项目也是这样布局的。这个项目很小，所以它现在只使用目录。下面的清单 3.2 显示了典型的 Nim 项目目录布局。C:\codeC:\code\ChatApp~/code/ChatAppsrctestsimagesdocssrc
...
❺	此目录包含所有 Nim 源代码文件，其中包含对 中包含的文件的测试。src
 

注释 项目目录布局| |良好的项目目录布局是非常有益的，特别是对于大型应用程序。最好尽早设置它。将应用程序源代码与测试分开意味着您可以轻松编写不冲突或不影响应用程序的测试代码。通常，分离也使代码导航更容易。|

现在在目录中创建一个文件。此文件将编译为可执行文件，并充当聊天应用程序的客户端组件。client.nimsrcclient

现在可以开始编写代码了。作为一个小测试，首先将以下内容写入新文件，然后保存。client.nim

echo("Chat application started")
为了编译您的新文件，请按照以下步骤操作。client.nim

打开一个新的终端窗口。
cd到项目目录中，通过执行 哪里 应该替换为项目目录的路径。cd ~/code/ChatApp~/code/ChatApp
通过执行 来编译文件。client.nimnim c src/client.nim
提示阿波里亚| |如果您使用的是Aporia，则只需按F4或在菜单栏中选择“工具>编译当前文件”即可编译当前选定的选项卡。也可以按 F5 进行编译和运行。|

如果您正确完成了所有操作，您应该在终端窗口中看到以下内容。

图 3.5.成功编译 .client.nim

ch03 successful compilation

提示输出目录| |默认情况下，Nim 编译器将在与编译的文件相同的目录中生成可执行文件。您可以使用该标志来更改此设置。例如，将可执行文件放在当前目录中。|-onim c -o:chat src/client.nimchat

假设编译成功，现在可以启动编译器创建的可执行文件。要开始执行，请使用以下命令。这应该在屏幕上显示“聊天应用程序已启动”，然后退出。./src/client
...
在 Nim 中，可以通过模块中定义的过程访问命令行参数。若要使用此过程，必须先导入模块。为此，请在文件顶部添加。然后，您可以扩展以读取第一个命令行参数，以便将代码更改为清单 3.3 中列出的代码。添加的代码以粗体显示。paramStrosparamStrosimport osclient.nimclient.nim
...
❶	这是使用模块中定义的过程所必需的。paramCountos
❷	确保用户已在命令行上指定了参数。
❸	过早停止应用程序，因为如果没有该参数，它将无法继续。
❹	检索用户指定的第一个参数并将其分配给新变量。serverAddr
❺	向用户显示消息，其中 是用户指定的地址。"Connecting to <serverAddr>"<serverAddr>
检查提供给可执行文件的参数数量始终很重要。该过程以整数形式返回参数数。上面的示例检查参数数是否为 0，如果是，它将调用该过程，并详细说明应用程序退出的原因。该过程退出应用程序，如果提供一条消息，它将首先显示该消息，然后退出并显示退出代码，告知操作系统应用程序失败。paramCountquitquit

当用户提供命令行参数时，该过程用于检索提供的第一个参数。使用索引 1，因为可执行文件名称存储在索引 0 处。然后，第一个命令行参数绑定到变量。paramStrserverAddr
...
示例 3.3 中的最后一行使用该过程在屏幕上显示附加到变量内容的字符串。该过程接受可变数量的参数，并在同一行上显示每个参数。echo"Connecting to "serverAddrecho
...
与在应用程序启动之前传递给应用程序的命令行参数不同，我们希望让用户能够实时发送消息，以响应他们从其他用户那里收到的消息。这意味着我们希望应用程序理想地做的是始终准备好从用户那里读取数据。

当应用程序在终端或命令行内运行时，可以在终端窗口中键入字符。应用程序可以通过标准输入流检索这些字符。就像在 Python 中一样，标准输入流可以通过变量访问。在 Nim 中，此变量在隐式导入的模块中定义，其类型为 。这意味着可以像从任何其他对象一样读取标准输入流。为从对象读取数据定义了许多过程，通常最有用的是从指定的 .stdinsystemFileFileFilereadLineFile

将清单 3.4 中的以下代码添加到 的底部，然后重新编译并运行它（您可以使用以下命令快速执行此操作）。client.nimnim c -r src/client.nim localhost
...
注意 字符转义序列| |示例 3.4 中的最后一行使用字符转义序列来显示字符。这需要转义，因为编译器会认为字符串文本已经结束。|"

您将看到您的应用程序不再立即退出，而是等待您在终端窗口中键入内容并按 Enter 键。完成此操作后，将显示一条消息，其中包含您在终端窗口中键入的文本。

从标准输入流读取将导致应用程序停止执行，应用程序将被阻止。完全读取请求的数据后，执行将恢复。在 的情况下，应用程序将保持阻塞状态，直到用户在终端中输入一些字符并按 Enter 键。当用户执行这些操作时，他们实际上是将一行文本存储到 的缓冲区中。stdin.readLinestdin

阻塞是大多数输入/输出调用的不幸副作用，可悲的是，这意味着您的应用程序在等待用户输入时将无法执行任何有用的工作。这实际上是一个问题，因为该应用程序需要主动保持与聊天服务器的连接，如果它正在等待用户在终端窗口中键入文本，则无法执行此操作。下面的图 3.13 显示了由此导致的问题。

图 3.9.客户端被无限期阻止导致的问题
...
终止应用程序实际上是确定当前正在执行哪一行代码的好方法。上面你可以看到，当应用程序终止时，client.nim 中的第 9 行正在执行，这对应于等待用户输入的阻塞调用。let message = stdin.readLine()readLine
...
❷	从线程返回的值不会立即可用，因此读取它必须由操作员显式完成。^
该过程返回一个字符串值。但是，当此过程在另一个线程中执行时，其返回值不会立即可用。为了解决这个问题，返回一个名为 的特殊类型。此类型保存生成的过程返回的值。readLinespawnFlowVar[T]

运算符可用于检索类型保存的值。但是一个对象可能并不总是包含一个值，它将开始时是空的，一旦生成的过程返回一个值，该对象就会包含它。当对象为空时，运算符将阻塞当前线程，直到对象不再为空。如果它首先不为空，则运算符将立即返回该值。^FlowVar[T]FlowVar[T]FlowVar[T]FlowVar[T]^FlowVar[T]^

这就是为什么上面的代码段实际上会以与前面的代码类似的方式运行。值得庆幸的是，您还可以使用该过程检查类型是否包含值。使用该过程将允许应用程序避免阻止行为。FlowVar[T]isReady

泛 型

泛型是 Nim 的一个功能，您将在第 9 章中详细介绍。现在，您需要知道的只是一个泛型类型。这允许它存储任何类型的值，存储的值的类型在方括号中指定。FlowVar[T]

例如，表达式返回一个类型，因为返回类型 是 ，并且 包装了生成的过程的返回值。spawn stdin.readLine()FlowVar[string]readLinestringFlowVar

将调用应用于任何返回 a 的过程将生成一个值：spawnstringFlowVar[string]
...
ch03 human protocol重要的是要记住，即使协议定义良好，仍然有很大的错误空间。消息可能只是未正确传输。这就是为什么解析给定消息的代码可以处理格式不正确的消息或不包含所需数据的消息至关重要的原因。我将在本节中向您展示的代码不会竭尽全力验证它收到的消息的有效性。但我鼓励您稍后花时间添加异常处理代码，以验证消息的有效性，并为代码用户提供更好的异常消息。
...
这只是一个非常简单的变体类型，只有两种状态。您还可以在变量类型的 case 语句中使用枚举类型。这是很常见的，并且在类型中使用。JsonNode

有7种不同的'。该类型是一个枚举，每种 JSON 值都有一个值。示例 3.10 提供了所有 7 个列表以及它们映射到的列表。JsonNodeKindJsonNodeKindJsonNodeKind

清单 3.10.JSON 值和类型之间的映射JsonNodeKind
...
解析任意 JSON 数据时，需要变体类型，因为编译器在编译时无法知道生成的 JSON 类型应该是什么。这仅在运行时知道。这就是过程返回一个类型的原因，该类型的内容因传递到过程中的 JSON 值的类型而异。parseJsonJsonNodeparseJson

清单 3.10 中显示的最后两个 JSON 值是集合。该类型表示 a 和 a 之间的映射。该种类存储 的列表。JObjectstringJsonNodeJArrayJsonNode

您可以使用运算符访问 的字段。它类似于数组和序列运算符，但将 a 作为其参数。该字符串确定要从 .要记住的一件重要事情是运算符返回一个值。JObject[][]stringJObject[]JsonNode
...
[注意]	事项kind
在 who's 字段 is 上使用字符串调用运算符将导致引发异常。[]JsonNodekindJObject	
那么如何从解析的字段中检索字段呢？使用将返回另一个 .除非该字段在解析后不存在，否则将引发异常。在上面的列表中，将返回的类型将是因为该字段包含值。您可以使用该过程检索值。每种类型都有一个“get”过程，如果它要返回的值的类型与类型不匹配，则每个“get”过程将返回默认值。usernameJsonNodedataJson["username"]JsonNodeusernameJObjectKeyErrorJsonNodedataJson["username"]JStringstringstringgetStrJsonNodeJsonNode

提示 “get”默认值| |可以覆盖“get”过程返回的默认值。要覆盖，请将默认情况下要返回的值作为第二个参数传递给过程，例如 。|node.getStr("Bad kind")

现在，您只需将此用户名分配给该类型的新实例。参见代码 3.11 它显示了完整的模块，已添加的新赋值以粗体显示。Messageprotocol

清单 3.11.将解析的数据分配给结果变量
...
您可能想知道 3.11 中的变量来自哪里。答案是，尼姆含蓄地为你定义了它。此变量在使用返回类型定义的所有过程中定义。resultresult

proc count10(): int =
  for i in 0 .. 10:
    result.inc
assert count10() == 10
这意味着您不需要重复定义变量，也不需要返回它。系统会自动为您返回该变量。有关详细信息，请查看第 2 章中的过程定义部分。resultresult

您应该尽可能快地测试代码。现在可以通过开始将新模块与客户端模块集成来执行此操作，但最好将代码作为单独的单元进行测试。协议模块是一个很好的代码单元，可以单独测试。测试模块时，最好测试每个导出的过程，以确保它们按预期工作。协议模块目前只导出一个过程 - 过程 - 因此您只需要为其编写测试。parseMessage

在 Nim 中测试代码有多种方法，最简单的方法是使用该过程。该过程在模块中定义。它类似于过程，它接受一个类型的参数，如果该参数的值为 .它与一个简单的方式不同，当您在发布模式下（通过标志）编译应用程序时，语句会优化出来，而语句则不会。doAssertdoAssertsystemassertbooleanAssertionFailedbooleanfalseassertassert-d:releasedoAssert

注意：发布模式| |默认情况下，Nim 编译器在调试模式下编译应用程序。在调试模式下，您的应用程序运行速度稍慢，但会执行某些检查，这些检查会为您提供有关可能意外引入程序的某些错误的更多信息。部署应用程序时，为了获得最佳性能，应使用将其置于发布模式的标志对其进行编译。|-d:release
...
3.4.3 生成 JSON
您已经能够成功解析 JSON，因此现在让我们继续生成 JSON。协议模块需要能够解析和生成消息。值得庆幸的是，生成 JSON 甚至比解析它更简单。

在 Nim 中，可以通过多种方式生成 JSON。一种方法是简单地创建一个字符串，其中包含与值连接的正确 JSON，就像在第一次测试中所做的那样。这将起作用，但容易出错，因为很容易错过 JSON 的某些语法元素。另一种方法是构造一个新的并使用运算符将其转换为字符串。让我们现在就这样做。首先定义一个新过程，然后使用运算符创建新对象。清单 3.13 显示了如何定义该过程。JsonNode$createMessage%JsonNodecreateMessage
...
注意 表构造函数语法| |您在上面的代码清单中看到的语法称为表构造函数。它只是数组构造函数的语法糖，即 与 相同。|{:}{"key1": "value1", "key2": "value2"}[("key1", "value1"), ("key2, "value2")]

该运算符非常强大，因为它可以将各种不同类型的值转换为适当的类型。这允许使用非常直观的语法创建 JSON。%JsonNode

按照惯例，它是用于将任何类型的值转换为值的运算符。对于 ，为其定义的运算符将生成所构建对象的有效 JSON 字符串文字表示形式。$stringJsonNode$JsonNode

添加回车符和换行符（某些操作系统使用它们来表示换行符）将在以后客户端和服务器组件需要接收消息时很有用。他们需要一种方法来确定新消息何时停止而另一条消息何时开始，本质上这些字符将是消息分隔符。实际上可以使用任何分隔符，但该序列已经在许多其他协议中常用，并且由Nim的网络模块支持。\c\l

就像该过程一样，您还应该为该过程添加测试。只需再次使用，即可确保输出符合预期。请记住在预期输出中包含 。下面的代码清单显示了一个可以执行的测试，将其添加到 的底部。parseMessagecreateMessagedoAssert\c\lprotocol.nim
...
接收的任何消息都需要发送给当前连接到它的所有其他消息。serverclient
...
|类型定义|名称| | | | | | | | | |MyType = objectinitMyTypeMyTypeRef = ref objectnewMyTypeRefMyTuple = tuple[x, y: int]initMyTuple
...
|程序|文件等效|描述 | | | |允许从远程端读取传入数据。对于 TCP 套接字，使用 UDP 套接字。| | | |将数据发送到套接字，允许将数据发送到远程端。对于 TCP 套接字，使用 UDP 套接字。| | | |将套接字连接到远程服务器。这通常仅用于 TCP 套接字。| | |无|将套接字绑定到指定的地址和端口。调用时，套接字将成为服务器套接字，其他套接字可以连接到它。这通常仅用于 TCP 套接字。|recvreadrecvrecvFromsendwritesendsendToconnectopenbindAddr
...
这是一种特殊的类型，它有很多名称，包括承诺、延迟和延迟在其他语言中。此类型充当最初未知的结果的代理，通常是因为其值的计算尚未完成。Future
...
ch03 async await execution

使用的过程代替运行事件循环，直到过程完成其执行。下面的表3.2显示了到目前为止您看到的所有不同异步关键字之间的比较。waitForrunForeverreadFiles

表 3.2.常见异步关键字之间的比较

|程序|直接|控制事件循环用例|描述 | | |是的|通常用于需要无限期保持活动状态的服务器应用程序。|永久运行事件循环。| | |是的|通常用于在特定异步过程完成其执行后需要退出的应用程序|运行事件循环，直到指定的将来完成。| | |是的|适用于需要精确控制事件循环的应用。和过程调用此。|侦听指定时间量的事件。| | |没有|安全丢弃期货。通常执行异步进程而不关心其结果。|将指定将来的回调属性设置为将适当处理异常的过程。| | |没有|用于执行另一个异步进程，其结果在 .|暂停异步进程的执行，直到指定的将来完成。|runForeverwaitForpollrunForeverwaitForasyncCheckawaitawait
...
3.6 小结
聊天应用程序的体系结构，包括客户端-服务器和对等模型与常见的 Internet 协议（如 TCP 和 UDP）之间的比较。
设置您的第一个 Nim 项目并对其进行编译，包括用于自定义编译过程的其他标志和可用于容纳这些标志的 Nim 配置文件。
使用该过程检索命令行参数。paramStr
使用 从标准输入流中读取行。readLine
通过 用于从另一个线程中的标准输入流读取数据来解决同步 I/O 的阻塞问题。spawn
使用该模块解析和生成 JSON。json
使用 编写简单的单元测试 .doAssert
使用模块和套接字通过网络传输数据。asyncnet
尼姆的模块系统。
