...
>提示 “get”默认值
可以覆盖“get”过程返回的默认值。要覆盖，请将默认情况下要返回的值作为第二个参数传递给过程，例如 。|node.getStr("Bad kind")
...
在 Nim 中，可以通过多种方式生成 JSON。一种方法是简单地创建一个字符串，其中包含与值连接的正确 JSON，就像在第一次测试中所做的那样。这将起作用，但容易出错，因为很容易错过 JSON 的某些语法元素。另一种方法是构造一个新的并使用运算符将其转换为字符串。让我们现在就这样做。首先定义一个新过程，然后使用运算符创建新对象。清单 3.13 显示了如何定义该过程。
JsonNode$createMessage%JsonNodecreateMessage
...
>注意 表构造函数语法
您在上面的代码清单中看到的语法称为表构造函数。它只是数组构造函数的语法糖，即 与 相同。|{:}{"key1": "value1", "key2": "value2"}[("key1", "value1"), ("key2, "value2")]
