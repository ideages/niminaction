...
5.6.3 编写文件并整理依赖关系.nimble

现在模块都已放置在正确的目录中，是时候创建文件了。您可以执行这将为您创建一个简单的。图 5.14 显示了生成清单 5.6 所示文件所需的问题和答案的示例。NimbleExample.nimblenimble initnimble initNimbleExample.nimble

图 5.13.命令nimble init

ch05 nimble init

清单 5.5.的开端NimbleExample.nimble
...
requires "nim >= 0.12.0"
在执行或将清单 5.6 的内容保存为 之后，您应该能够执行 。这应该成功安装您的软件包！nimble initNimbleExample.nimblenimble install

这就是创建 Nimble 包是多么简单。但是创建 Nimble 软件包只是开发 Nimble 软件包的一小步。软件包在不断发展，需求也会随之变化，那么 Nimble 如何在开发过程中帮助我们呢？

在开发软件包时，您可能有一天会意识到您需要另一个 Nim 库的功能。在许多情况下，此库将是一个 Nimble 包。例如，您可能希望为非常大的整数创建一个版本，这些整数大于 Nim 标准库中可以存储的最大整数类型。该包提供此功能。addbigints

打开包中的文件并对其进行更改，使其内容与清单 5.7 中的内容相同。更改以粗体突出显示。math.nimNimbleExample

清单 5.6.在模块中使用包bigintsmath

**import bigints**   ❶
proc add*(a, b: int): int = a + b
**proc add\*(a, b: BigInt): BigInt = a + b**   ❷

❶	从包中导入模块。无需显式声明包名称和模块名称。bigintsbigints
❷	为模块中定义的类型定义过程。addBigIntbigints
现在尝试通过执行 来编译它。编译器应输出类似于 的内容。这指向导入模块的代码行。编译失败的原因是尚未安装包。立即通过执行 安装它。现在再次编译。这次编译应该会成功。nim c NimbleExample/mathmath.nim(1, 8) Error: cannot open 'bigints'bigintsbigintsnimble install bigintsNimbleExample/math

这是否意味着包的每个用户都需要手动安装包？目前，是的。但这就是文件中的依赖规范的用武之地。NimbleExamplebigintsNimbleExample.nimble

使用 Nim 编译器编译任何 Nim 源代码时，您使用 Nimble 安装的每个软件包都将可用于该源代码。这就是为什么在安装包后立即导入模块的原因。bigintsbigints

全局敏捷包和 Nim 编译器

默认情况下，使用 Nimble 安装软件包时，该软件包将安装到当前用户的 Nimble 软件包存储区中，该存储区位于 .每次使用 Nim 编译器编译 Nim 模块时，该模块都可以导入属于 Nimble 包存储区中任何包的任何模块。~/.nimble/

但是，如果安装了同一软件包的两个版本怎么办？在这种情况下，Nim将只使用最新的。

现在尝试使用 Nimble 编译相同的文件。Nimble支持一个方便的命令，它完全按照Nim编译器的作用，编译指定的文件。现在执行并记下结果。您可能会对失败感到惊讶，但它说明了直接使用 Nim 编译器编译和使用 Nimble 编译之间的关键区别。Nimble 不允许您导入任何未在项目文件中指定为依赖项的模块。cnimble c NimbleExample/math.nimble

让我们更改文件，使其包含包作为依赖项。清单 5.8 显示了文件现在的样子，差异以粗体突出显示。NimbleExample.nimblebigintsNimbleExample.nimble

示例 5.7.添加对包的依赖关系bigints
...
清单 5.8 中的依赖关系没有指定对该包的版本的要求。在这种情况下，Nimble 将尝试安装该库的最新标记版本，即假设尚未安装该库。bigints

尖端依赖| |您可以在包名称后指定，例如 。这将使 Nimble 始终使用该包的最新版本编译您的包。下面的图 5.15 显示了指定和不指定之间的区别。#headbigints#head#head

图 5.14.与无 ch05 灵活安装标签之间的区别 |#head#head

一旦你更改了文件以匹配清单 5.8，你应该能够使用 Nimble 成功编译模块。如果检测到软件包未安装，Nimble 甚至会自动为您安装该软件包！下面的图 5.16 显示了软件包是否已安装之间的区别。NimbleExample.nimblemathbigintsnim cnimble cbigints

图 5.15. 与。nim cnimble c

ch05 nimble c nim

您现在应该对 Nimble 如何处理依赖项有了基本的了解，并且应该具备创建更多 Nimble 包所需的知识。仍然缺少一条知识，那就是发布 Nimble 包所涉及的过程。

在进入下一节讨论发布 Nimble 包之前，我想给你一个快速的挑战。在此包的某些模块中为此 Nimble 包编写一些简单的测试，然后使用 Nimble 的命令运行这些测试。这应该可以帮助您入门：，指定标志将意味着生成的可执行文件在编译后自动运行。请记住将您的测试放在语句下，此语句可确保仅在直接编译模块时执行其主体中的任何代码。应该使用它的原因是，在应用程序中导入模块时不会执行测试。cnimble c -r NimbleExample/math-rwhen isMainModule:mathmath
...
ch05 github new repo

指定“敏捷示例”作为“存储库名称”，然后单击绿色的“创建存储库”按钮。然后，您将看到另一个网页，该网页将让您知道如何在命令行上创建存储库。网页上的指令非常通用，清单 5.9 显示了类似于网页上的命令，但这些命令是为成功将包上传到 GitHub 而定制的。立即执行这些命令。NimbleExample

清单 5.8.用于将包上载到 GitHub 的命令。NimbleExample
...
❶	记得改成你的Github用户名！<your-user-name>
成功执行这些命令后，导航到 https://github.com//NimbleExample 应会显示文件列表。这些文件应包括文件、目录及其内容。NimbleExample.nimbleNimbleExample

只剩下一件事要做。该软件包是公开的，但 Nimble 尚未找到它，因为它尚未添加到其软件包列表中。这意味着您将无法通过执行来安装它。nimble install NimbleExample
...
在发布包之前，最好确保可以成功安装包。在包的目录中执行以验证是否可以成功安装。nimble install

然后，包就可以发布了。您可以手动创建拉取请求，也可以使用 Nikimble 的命令。立即执行并按照屏幕上的提示进行操作。publishnimble publish

该过程有些复杂，因为它需要您为 Nimble 创建新的 GitHub 访问令牌。但是一旦你这样做了，它就会大大简化发布 Nimble 包的过程。

当您的软件包被接受并添加到软件包列表中时，您将能够通过执行 来安装它。nimble install NimbleExample

请记住，发布 Nimble 包只执行一次。在开发新版本包时，无需再次发布包，而是标记版本，您将在下一节中找到。

5.8 开发一个灵活的软件包
软件项目通常被赋予版本来确定其状态。软件不断发展，新的开发使用不断增加的版本号进行标记。灵活的软件包也不例外。该软件包以版本开始其生命，如果继续开发，它有一天可能会达到版本甚至.版本可帮助用户区分和识别包的不同状态。NimbleExample0.1.01.010.3

包的版本信息存储在包的文件中。它是使用密钥指定的。版本必须至少包含一个数字，多个数字必须用句点分隔。例如。.nimbleversionversion = "1.42.5"

5.8.1 赋予版本号含义
版本号的分配和递增方式不同。在某些情况下，版本号除了表示版本比版本新之外几乎没有其他意义。在其他情况下，例如使用语义版本控制，版本号会告诉您有关不同版本软件的 API 兼容性的更多信息。1.00.5
...
递增文件中的版本号。.nimble
将这些更改提交到存储库中。例如：。git commit -am "Version 0.1.2"
标记您刚刚进行的提交，使用新版本号作为标记名称。例如：。git tag v0.1.2
将更改上传到远程存储库，确保也上传标签。例如：。git push origin master --tags
首先执行步骤 1 非常重要。如果标记的名称与标记对应的历史记录点的文件中指定的版本不匹配，则会出现不一致，Nimble 将拒绝安装软件包。.nimble

标记上述版本所需的步骤特定于 Git。你会发现，为了开发 Nimble 包，你至少需要对 Git 或 Mercurial 有非常基本的了解。

5.9 小结
敏捷不是开发 Nim 软件的必要条件，但它是一个有用的工具，可让您访问数百个软件包，从而使实现软件的工作更容易。
