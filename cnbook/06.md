...
createThread[string](threads[0], countData, data)     # <4>
createThread[string](threads[1], countData, data)     # <4>
...
<4> 将从索引 `i` 开始的字符从字符串 `line` 复制到第二个参数中指定的字符串，直到line[i]==。返回的值是捕获的字符数
...
⓫ 创建一个新线程来执行 `parseChunk` 过程，传递一个包含可解析片段的缓冲区片段。将派生返回的 `FlowVar[string]` 添加到响应列表中。
⓬ 将未解析的片段部分分配给缓冲区的开头。
⓭ 遍历每个响应。
