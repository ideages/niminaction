...
将代码保存在“createDatabase.nim”文件中的*清单7.11*中。然后通过执行“agic c-r src/createDatabase”编译并运行它。您应该会看到“Database created successfully！”（数据库创建成功！）消息和tweeter目录中的一个`tweeter.db`文件。
