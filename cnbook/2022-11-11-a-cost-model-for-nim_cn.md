...
把一个系统设计的如此完美，以至于没人需要优秀（好），是不可能的

设计一个完美的系统，这是不可能的，，没有人需要了。

太完美了，以至于没有人需要好，是不可能的。

设计完美的系统，
把系统设计完美到，没有人需要好，是不可能的。

> 设计一个完美无瑕的系统，无人问津，子虚乌有。

> 好到没有人需要好的，那么优秀的系统，是设计不出来的。

> 设计的系统，完美无瑕到众望所归，纯属子虚乌有。
设计的系统，完美无瑕到众望所归，纯属白日做梦，痴心妄想
登峰造极

设计一个 完美到没有人需要的系统 是不可能的。

设计一个如此完美的系统是不可能的，以至于没有人需要优秀
> 设计一个完美到无人需要的系统是不可能的好的。
>
> --- T. S. Eliot T·S·艾略特（英国诗人）
英国（美国出生）评论家、剧作家和诗人（1888 - 1965）
{==+==}
...
{==+==}
本篇博客是个开始，具体成本模型的实现， 可在 “Nim-devel”（即Nim版本2）获得。
{==+==}
...
{==+==}
此实现是为**嵌入式硬实时系统**设计的。一般来说，假设您有足够的内存（大约为**64kB**），则支持Nim的所有语言特性——包括异常处理和基于堆的存储。这些功能的实现也可以在裸机上运行，无需操作系统。
{==+==}
...
为什么关注嵌入式、硬实时系统？因为当你做好这些工作时，你也可以做好其他的事情！所使用的算法忽略了堆大小：例如， Nim 的内存管理在 64kB 大小的堆上运行良好，但也可以扩展到 16GB 的堆。
{==+==}
...
在Nim版本2中，可以在线程之间有效地共享内存，而无需复制。分配和解除分配的复杂性为 O(1)，Nim的默认分配器是完全无锁的。碎片化程度较低（平均值为15%，最坏情况为25%），因为它基于[TLSF](http://www.gii.upv.es/tlsf/)算法。无锁思想来自[mimalloc](https://github.com/microsoft/mimalloc).
{==+==}
...
从根“x”开始的子图的破坏成本是O（N），其中N是子图中的节点数。非循环数据的子图在其根的引用计数达到0时立即被销毁。
...
{==+==}
循环数据很难推理，最好避免。然而，如果您最终创建了循环数据，则系统始终保持“确定性”和响应性。您可以通过新的API影响循环收集器：
{==+==}
...
{==+==}
这样的想法是，你可以在程序 “方便” 的时候安排一次循环收集。就是说，在程序当前不忙时。然而，在我对Nim异步事件循环的实验中，我发现这样做没有好处。这里要吸取的教训是“放松，你会没事的”。
{==+==}
...
{==+==}
从版本2开始， `of` 运算符（也在 `except E as ex` 构造中隐式使用）的速度终于达到了应有的速度：它是一个范围检查，然后是内存提取。成本为 O(1)。

Nim 的异常基于支持运行时多态性的老式类型层次结构。不同的异常类的大小可能不同。因此，在堆上分配异常。但是，可以预先分配它们。标准库还没有做到这一点 - 欢迎拉取请求！
{==+==}
...
###基于Goto的异常处理
当您编译为C代码时，异常处理是通过设置一个内部错误标志来实现的，该标志在每次可以引发的函数调用后都会被查询 `setjmp` 不再使用。要提高编译器推理哪个调用“可以引发”的能力，请明智地使用  `.raises: []`  注释。错误路径与成功路径交织在一起，产生了指令缓存的优点和缺点。

所涉及的成本约为一次呼叫后的2-4条机器指令。（对于最常见的体系结构，有已知的方法可以将其进一步优化为一条指令。）这一开销很烦人，但至少针对尾部调用进行了优化。
...
当您编译为C++代码时，将使用异常处理的C++实现——通常它基于异常处理表。
...
有传言称，基于表的异常实现缺乏“可预测”的性能，因此不应用于硬实时系统。如果这些传言仍然属实，那么应该首选基于goto的异常处理。
{==+==}
...
这些映射到存储的线性部分，直接嵌入到父集合中。这意味着如果不涉及 `ref` 或 `ptr` 间接寻址，它们将在堆栈上分配——这不是什么新鲜事，对于每个Nim版本和内存管理模式都是如此。
...
{==+==}
相反，Nim中的字符串 *字面量* 不会导致分配，并且可以在  O(1) 操作中进行浅层复制。
如果您不同意这种设计选择，并且希望有字符串执行SSO，则可以使用外部包（[sostrings](https://github.com/planetis-m/ssostrings)，[最短字符串](https://github.com/metagn/shorteststring)).
{==+==}
...
{==+==}
### 哈希表
Nim的大多数标准库集合都是基于哈希的，并且只提供 “平均” 情况下的O（1）行为。通常，这对于硬实时设置来说不够好，因此必须避免这些设置。事实上，在 Nim 的整个历史中，人们发现了这些数据结构上表现不佳的案例。相反，可以使用基于BTree的容器，为查找/插入/删除提供O（log N）操作。可以在[此处]找到一个可能的实现(https://github.com/nim-ang/fusion/blob/master/src/fusion/btreetables.nim).
{==+==}
