...
编译上述代码列表时，编译消息中将显示消息“正在生成字符串”。这是因为fillString过程是在编译时执行的。
编译时执行有一些限制，包括：
...
newProc过程生成对过程建模的必要AST节点。您可以通过指定名称、参数、返回类型和过程主体来自定义过程。
